  ...
  if (config.schema !== undefined) {
    for (let i = 0; i < config.schema.length; i++) {
      ...
      for (let j = 0; j < config.schema[i].property.length; j++) {
        ...
        const SQLTYPE = ['TINYINT', 'SMALLINT', 'MEDIUMINT', 'INT', 'BIGINT', 'FLOAT', 'DOUBLE', 'DECIMAL', 'NUMERIC', 'BIT', 'DATE', 'TIME', 'DATETIME', 'TIMESTAMP', 'YEAR', 'CHAR', 'VARCHAR', 'BINARY', 'VARBINARY', 'BLOB', 'LONGBLOB', 'TEXT', 'ENUM', 'SET'];
