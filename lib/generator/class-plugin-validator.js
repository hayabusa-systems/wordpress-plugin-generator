...
class ${upper_schema_name}_Validator
{
    ...
    public static function validate($entity) : bool
    {
        ...
        if (get_class($entity) != '${upper_schema_name}') {
            self::$errors['general'][] = '予期しないオブジェクトが渡されました。';
            return false;
        }
`;
    s.property.forEach(p => {
      ...
      if (p.required) {
        ...
        if (empty($entity->${p.name})) {
            self::$errors['${p.name}'][] = '${label}は入力必須です。';
        }
`
      }
      if (p.form) {
        ...
        if (strlen($entity->${p.name}) > ${rule.maxlength}) {
            self::$errors['${p.name}'][] = '${label}は最大${rule.maxlength}バイトまでしか入力できません。';
        }
`
          }
        ...
        if ($entity->${p.name} < ${rule.min}) {
            self::$errors['${p.name}'][] = '${label}は${rule.min}以上の数値を入力してください。';
        }
`
          }
        ...
        if ($entity->${p.name} < ${rule.max}) {
            self::$errors['${p.name}'][] = '${label}は${rule.max}以下の数値を入力してください。';
        }
`
          }
          if (rule.kana) {
        ...
        if (preg_match(REGEX_KANA_PATTERN, $entity->${p.name}) != 1) {
            self::$errors['${p.name}'][] = '${label}の入力形式が不正です。カタカナを入力してください。';
        }
`
          }
          if (rule.mailaddress) {
        ...
        if (preg_match(REGEX_MAIL_PATTERN, $entity->${p.name}) != 1) {
            self::$errors['${p.name}'][] = '${label}の入力形式が不正です。メールアドレスを入力してください。';
        }
`
          }
          if (rule.phone) {
        ...
        if (preg_match(REGEX_PHONE_PATTERN, $entity->${p.name}) != 1) {
            self::$errors['${p.name}'][] = '${label}の入力形式が不正です。 電話番号を入力してください。';
        }
`
          }
          if (rule.url) {
        ...
        if (preg_match(REGEX_URL_PATTERN, $entity->${p.name}) != 1) {
            self::$errors['${p.name}'][] = '${label}の入力形式が不正です。 URLを入力してください。';
        }
`
          }
          if (rule.zipcode) {
        ...
        if (preg_match(REGEX_ZIPCODE_PATTERN, $entity->${p.name}) != 1) {
            self::$errors['${p.name}'][] = '${label}の入力形式が不正です。 郵便番号(数字7桁または数字3桁-数字4桁)を入力してください。';
        }
`
          }
        }
