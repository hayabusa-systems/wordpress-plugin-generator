...
`;
    if (plugin.upload_dir) {
      plugin_file_code += 
`include_once plugin_dir_path(dirname(__FILE__)) . 'library/file.php';
    ...
    if (s.embeded) {
      s.embeded.forEach(e => {
        ...
        if (!empty($form['${object_name}'])) {
            foreach ($form['${object_name}'] as $i => $v) {
                $is_add_data = false;
                $child = new \\Plugin\\${plugin_name}\\Entity\\${upper_embeded_name}();`;
        config.schema.filter(s => s.name == e.name).forEach(is => {
          is.property.forEach(p => {
            const form_name = (p.form && p.form.name) ? p.form.name : p.name;
            if (p.form && p.form.type == "file") {
              const form_file_name_prefix = object_name + '_';
              const form_file_name_suffix = '_' + form_name + "_file";
              plugin_file_code += `
                if (isset($_FILES['${form_file_name_prefix}' . $i .'${form_file_name_suffix}']) && $_FILES['${form_file_name_prefix}' . $i .'${form_file_name_suffix}']['size'] > 0) {
                    $child->${p.name} = File::upload_image_file('${form_file_name_prefix}' . $i .'${form_file_name_suffix}');
                    $is_add_data = true;
                } else {
                    if (isset($v[$i]['${form_name}'])) {
                        $child->${p.name} = $v[$i]['${form_name}'];
                        $is_add_data = true;
                    }
                }`;
            } else if (p.form && p.form.type == "checkbox") {
              plugin_file_code += `
                if (isset($v['${form_name}'])) {
                    foreach ($v['${form_name}'] as $check) {
                        $child = new \\Plugin\\${plugin_name}\\Entity\\${upper_embeded_name}();
                        $child->${p.name} = $check;
              ...
              plugin_file_code += `
                if (isset($v['${form_name}'])) {
                    if (is_string($v['${form_name}']) && empty($v['${form_name}'])) {
                        // nothing
                    } else {
                        $child->${p.name} = $v['${form_name}'];
                        $is_add_data = true;
                    }
                }`;
            }
