...
class ${upper_schema_name}_Form
{
     ...
     * @since 1.0.0
     */
    public static function convertToEntity($form, $prefix = ""): object
    {
    ...
    s.property.forEach(p => {
      ...
      if (p.form && p.form.type == "file") {
        ...
        plugin_file_code += `
        if (isset($_FILES[$prefix . '${form_file_name}']) && $_FILES[$prefix . '${form_file_name}']['size'] > 0) {
            $entity->${p.name} = File::upload_image_file($prefix . '${form_file_name}');
        } else {
    ...
    if (s.embeded) {
      s.embeded.forEach(e => {
        ...
        if (e.form && e.form.type == "checkbox") {
          ...
          plugin_file_code += `foreach ($form['${object_name}'] as $i => $v) {
                $search_keys = array_filter(array_keys($_FILES), function ($v) use ($i) {
                    $ret = strpos($v, "${object_name}_" . $i . "_");
                    return (gettype($ret) == "integer" && $ret == 0);
                });
                if (!empty(array_filter($v)) || (!empty($search_keys) && $_FILES[$search_keys[array_keys($search_keys)[0]]]["size"] > 0)) {
                    $entity->${property_name}[] = \\Plugin\\${plugin_name}\\Form\\${upper_embeded_name}_Form::convertToEntity($v, "${object_name}_" . $i . "_");
                }`;
        }
        plugin_file_code += `
            }
