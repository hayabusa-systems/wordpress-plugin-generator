...
`;
        ...
        $repository = new \\Plugin\\${plugin_name}\\Repository\\${upper_embeded_name}_Repository();`;
            if (e.update_type && e.update_type == "drop_insert") {
                plugin_file_code += `
        $drop_data = $repository->findBy(array('product_id' => (int)$entity->id));
...
`;
        s.property.filter(p => p.loaded).forEach(p => {
            const loaded_name = p.loaded[0].toUpperCase() + p.loaded.slice(1);
            plugin_file_code += `
        $repository = new \\Plugin\\${plugin_name}\\Repository\\${loaded_name}_Repository();`;
            plugin_file_code += `
        foreach ($entities as $entity) {
            $entity->${p.loaded} = $repository->findById($entity->${p.name});
        }`;
        ...
        $repository = new \\Plugin\\${plugin_name}\\Repository\\${upper_embeded_name}_Repository();`;
            plugin_file_code += `
        $entity->${property_name} = $repository->findBy(array('${e.foreign_key}' => (int)$entity->id));`;
        });
    plugin_file_code += `
        return $entity;
    }
