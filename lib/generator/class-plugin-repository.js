...
`;
        s.embeded.forEach(e => {
            ...
            plugin_file_code += `
        include_once plugin_dir_path(dirname(__FILE__)) . 'repository/${e.name}-repository.php';
        $repository = new \\Plugin\\${plugin_name}\\Repository\\${upper_embeded_name}_Repository();`;
            if (e.update_type && e.update_type == "drop_insert") {
                plugin_file_code += `
        $drop_data = $repository->findBy(array('product_id' => $entity->id));
...
`;
        ...
        $repository = new \\Plugin\\${plugin_name}\\Repository\\${upper_embeded_name}_Repository();`;
            plugin_file_code += `
        $entity->${property_name} = $repository->findBy(array('${e.foreign_key}' => $entity->id));
        return $entity;`
        });
    plugin_file_code += `
    }
    ...
    public function findBy(array $condition)
    {
        $entities = parent::findBy($condition);
`;
        s.embeded.forEach(e => {
            let upper_embeded_name = e.name[0].toUpperCase() + e.name.slice(1);
            let property_name = utils.convertSnakeCase(utils.convertPruralForm(e.name));
