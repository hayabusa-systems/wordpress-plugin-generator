...
class Base_Repository
{
    ...
    public function save($entity)
    {
        ...
        if ($entity->id && $entity->id > 0) {
            ...
            $value_property.substr(0, -1);
            $condition.substr(0, -1);
            $value_property .= ")";
            $condition .= ")";
            $query = $wpdb->prepare(
            ...
            $condition.substr(0, -1);
            $query = $wpdb->prepare(
                "UPDATE $this->table_name $condition WHERE id = $entity->id;",
                $conditions
            );
            $result = $wpdb->query($query);
            if ($result === false) {
    ...
    public function findById(int $id)
    {
        ...
        return $result == null ? null : new $this->class_name($result);
    }
 ...
 */
namespace Plugin\\${upper_plugin_name}\\Repository;
require_once plugin_dir_path(__FILE__) . 'includes/repository/base-repository.php';
...
class ${upper_schema_name}_Repository extends Base_Repository
{
    ...
    public function __construct()
    {
        ...
        $this->class_name = "${upper_schema_name}";
    }
}
`;
