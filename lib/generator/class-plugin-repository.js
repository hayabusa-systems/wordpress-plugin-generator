...
class Base_Repository
{
    ...
    public function __construct()
    {
        ...
        $this->log = \\Plugin\\ShizuokaDB\\Library\\Log::getInstance();
    }
    ...
    public function findBy(array $option)
    {
            ...
            foreach ($option['condition'] as $key => $cond) {
                ...
                case 'array':
                    if (gettype($cond[0]) == 'integer') {
                        $ds = implode(',', array_fill(0, count($cond), '%d'));
                    } else {
                        $ds = implode(',', array_fill(0, count($cond), '%s'));
                    }
                    $query .= $this->table_name . '.' . "$key in ($ds)";
                    $values = array_merge($values, $cond);
                    break;
                }
                $query .= " AND ";
            }
        ...
        $this->log->debug('Row is ' . count($result));
        $list = array();
...
`;
    ...
    if (s.embeded) {
        s.embeded.filter(e => e.autoload === undefined || e.autoload).forEach(e => {
