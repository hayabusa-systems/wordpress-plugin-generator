...
EOD;
        foreach ($this->label_map[$short_class_name] as $property => $label) {
            ...
            if (preg_match('/\.[jpg|jpeg|png|gif]$/', $after_entity->{$property}) > 0) {
                $display_value = '<img src="' . $after_entity->{$property} . '">';
            }
            echo <<< EOD
                    <tr class="form-field">
                        <th scope="row">
...
EOD;
    ...
    private function _display_update($entity, $before_entity, $after_entity)
    {
        ...
        echo <<< EOD
		<div class="wrap">
			<h1 id="approval">$object_name更新承認</h1>
			<p>以下の内容で申請がありました。</p>
			<table class="form-table" role="presentation">
				<tbody>
                    <tr class="form-field">
...
EOD;
        foreach ($this->label_map[$short_class_name] as $property => $label) {
            ...
            preg_match('/\.[jpg|jpeg|png|gif]$/', $after_entity->{$property}) > 0) {
                $before_display_value = '<img src="' . $before_entity->{$property} . '">';
                $after_display_value = '<img src="' . $after_entity->{$property} . '">';
            }
            echo <<< EOD
                    <tr class="form-field">
                        <th scope="row">
