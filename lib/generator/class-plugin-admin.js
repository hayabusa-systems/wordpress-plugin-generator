...
class ${upper_schema_title}_Controller
{
    ...
    public function index()
    {
        ...
        <div class="wrap">
            ...
            <a href="./admin.php?page=mie_food_db_company_edit" class="page-title-action">新規追加</a>
        ...
        s.property.forEach(p => {
          ...
          if (p.form && p.form.type) {
            if (p.form.type == "select") {
              plugin_file_code += indent.repeat(8) + '<select name="' + p.name + '" id="' + p.name + '">\n';
...
`;
            ...
            plugin_file_code += indent.repeat(8) + '<input type="number" name="' + p.name + '" id="' + p.name + '" value="$entity->' + p.name + '"';
            if (p.required) {
              plugin_file_code += ' aria-required="true"';
            }
            ...
            plugin_file_code += indent.repeat(8) + '<textarea name="' + p.name + '"';
            if (p.required) {
              plugin_file_code += ' aria-required="true"';
            }
            plugin_file_code += '>$entity->' + p.name + '</textarea>';
          } else {
            plugin_file_code += indent.repeat(8) + '<input type="text" name="' + p.name + '" id="' + p.name + '" value="$entity->' + p.name + '"';
            if (p.required) {
              plugin_file_code += ' aria-required="true"';
            }
...
`;
    fs.promises.writeFile(
        './dist/' + plugin.name + '/admin/controller/class-' + upper_schema_title + '-controller.php',
        plugin_file_code)
        .catch((err) => {
          console.error(err.toString());
        });
