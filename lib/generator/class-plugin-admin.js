...
class ${upper_schema_title}_Controller
{
    ...
    protected function check_credential(): bool {
        if (!isset($_POST[self::CREDENTIAL_ACTION]) || !isset($_POST[self::CREDENTIAL_NAME])) {
            return false;
        }
        if(\\check_admin_referer(self::CREDENTIAL_ACTION, self::CREDENTIAL_NAME) === false) {
            return false;
        } else {
            return true;
        };
...
`;
      s.admin.list.column.forEach(c => {
          const convertConditions = s.property.filter(p => p.name == c.key)[0].form;
...
`;
              ...
              <div id="preview${drag_drop_zone}">
                <img src="$upload_base_url$entity->${p.name}" />
              </div>
              <script>
