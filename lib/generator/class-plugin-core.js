...
define('${all_upper_plugin_name}_LOGPATH', __DIR__ . '/log/${plugin_name}.log');
define('${all_upper_plugin_name}_LOGLEVEL', '0');
...
function Run_${all_lower_plugin_name}()
{
    include_once plugin_dir_path(dirname(__FILE__)) .
            '${plugin_name}-' . ${all_upper_plugin_name}_VERSION . '/includes/class-${plugin_name}.php';
...
const REGEX_HIRAGANA_PATTERN = '/^[ぁ-?][ぁ-?ー 　]*$/u';
...
`;
  plugin.const.forEach(v => {
    if (typeof(v['value'][0]) == "string") {
      ...
      plugin_file_code = plugin_file_code.slice(0, -2)
      plugin_file_code += ');';
    }
  });
  if (plugin.approval) {
