...
// Regex Pattern
const REGEX_KANA_PATTERN = '/^[ァ-ヶ][ァ-ヶー 　]*$/';
const REGEX_MAIL_PATTERN = '/[\w\-._]+@[\w\-._]+\.[A-Za-z]+/';
const REGEX_PHONE_PATTERN = '/^0\d{2,3}-?\d{1,4}-?\d{4}$/';
const REGEX_URL_PATTERN = '/https?:\/\/[-_.!~*()a-zA-Z0-9;\/?:@&=+$%#纊-黑亜-熙ぁ-んァ-ヶ]+/';
const REGEX_ZIPCODE_PATTERN = '/^\d{3}-?\d{4}$/';

`;
  plugin.const.forEach(v => {
    plugin_file_code += 'const ' + all_upper_plugin_name + '_' + v['name'] + ' = ["' +  v['value'].join('","') + '"];\n';
  });
